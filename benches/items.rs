...
// fn get_items() {
//     let client = api::Client::new(api::BASE_URL.to_string(), reqwest::Client::new());

//     // 1.3753s
//     // let a = client.get_item_by_id(26446070).unwrap();

//     let top_stories: Vec<u32> =
//         block_on(client.get_top_stories("orderBy=\"$key\"&limitToFirst=25")).unwrap();

//     let items: Vec<ListItem> = {
//         let mut vec = Vec::new();
//         for id in top_stories {
//             vec.push(block_on(client.get_item_by_id(id, "")).unwrap())
//         }
